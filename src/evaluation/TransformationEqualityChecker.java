...
/**
 * Agent for checking, whether a series of transformations represent an equal or
 * non-equal model structure modification. The equality check means, whether the
 * transformations made solely structural difference, but kept the same model's
...
public class TransformationEqualityChecker {
	 ...
	 * Compares given models, using entity-sets and relationships mappings between
	 * the models. Searches for specific scenarios, when models are different in
	 * structure but equal in ability to store data. Returns only those
	 * transformations (out of the 'transformationsMade' list) which do not play any
	 * role in none of the equality scenarios.
	 *
	 * @param modelBefore         = student's model before transformation list
	 *                            application
	 * @param modelAfter          = student's model after transformation list
	 *                            application
	 * @param transformationsMade = instances of the transformations applied
	 * @param entitySetMapping    = entity-set mapping between the old and the new
	 *                            model
	 * @param relationshipMapping = relationship mapping between the old and the new
	 *                            model
	...
	/**
	 * Iterates over relationships of one given model, searching for structural
	 * difference. When it finds one, checks whether it's one of the known equality
	 * cases. If yes, uses model elements mappings and given transformations
	 * references to get the transformation instances which are responsible for the
	 * equality scenario.
	 *
	 * @param thisModel           = entity relationship model to iterate over
	 * @param otherModel          = the opposite entity relationship model
	 * @param transformationsMade = see
	 *                            {@link #getNonEqualTransformations(ERModel, ERModel, List, Map, Map)}
	 * @param entitySetMapping    = see
	 *                            {@link #getNonEqualTransformations(ERModel, ERModel, List, Map, Map)}
	 * @param relationshipMapping = see
	 *                            {@link #getNonEqualTransformations(ERModel, ERModel, List, Map, Map)}
	 * @param isTransformed       = whether the 'thisModel' (model to iterate over)
	...
	private static List<Transformation> getEqualTransformationsByRelationships(ERModel thisModel, ERModel otherModel, List<Transformation> transformationsMade,
		...
		nextRel: for (Relationship relationship : thisModel.getRelationships()) {
			EntitySet[] oppositeEntitySets = new EntitySet[relationship.getSides().length];
			int i = 0;
			boolean allEntitySetsAreMapped = true; // meaning all entity sets have non-null mapping image, or their mapping image
													// wasn't removed due to transformations
			...
			if (relationship instanceof Association) {
				...
				if (association.isBinary() && Enums.CARDINALITY_MANY.equals(association.getFirstSide().getRole()) && Enums.CARDINALITY_MANY.equals(association.getSecondSide().getRole())) {
					...
					for (EntitySet joiningEntitySet : otherModel.getEntitySets()) {
						...
						// joining entity set (from other model) doesn't have an existing mapping image
						// in the first model (if it does, it wasn't created due to transformation)
						if (optionalImage1Association.isPresent() && optionalImage2Association.isPresent()
