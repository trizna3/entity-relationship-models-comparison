...
public class RelationshipEvaluator implements ISpecificEvaluator {
	...
	private void evaluateRelationshipsAssymetric(ERModel model1, ERModel model2, Mapping mapping) {
		...
		rel1: for (Relationship relationship1 : model1.getRelationships()) {
			rel2: for (Relationship relationship2 : model2.getRelationships()) {
				if (!usedRelationships.contains(relationship2) && ERModelUtils.relationshipsAreEqual(relationship1, relationship2, mapping, false)) {
			...
			this.c++;
			rel2: for (Relationship relationship2 : model2.getRelationships()) {
				if (!usedRelationships.contains(relationship2) && ERModelUtils.relationshipsAreEqual(relationship1, relationship2, mapping, true)) {
					usedRelationships.add(relationship2);
					continue rel1;
				}
			}
